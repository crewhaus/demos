...
```bash
bun install
bun run compile:hello-prochat                          # writes dist/agent.ts
ANTHROPIC_API_KEY=sk-ant-... bun run run:hello-prochat # opens REPL
```
...
Recompile (`bun run compile:hello-prochat`) after any change to the spec.
