...
```bash
CREWHAUS_TRACE=json bun run run starters/cli | jq -c .
```
...
CREWHAUS_METRICS=stdout bun run run starters/cli

# Atomic Prometheus textfile writes:
CREWHAUS_METRICS=textfile:/var/lib/node_exporter/crewhaus.prom bun run run starters/cli

# Pull-based HTTP endpoint:
CREWHAUS_METRICS=http:9464 bun run run starters/cli   # GET http://localhost:9464/metrics
```
...
```bash
CREWHAUS_COST_TRACKING=1 bun run run starters/cli
# Optionally annotate with tenant id (managed-shape deployments):
CREWHAUS_TENANT_ID=acme bun run run starters/cli
```
...
- **Correlate a runtime trace event back to the spec line that caused it.** [GETTING-STARTED.md § Tracing a request across YAML, IR, and trace](https://github.com/crewhaus/docs/blob/main/GETTING-STARTED.md#tracing-a-request-across-yaml-ir-and-trace) — two worked walkthroughs (permission denial; eval-driven prompt mutation) showing YAML, IR, and trace events side by side.
